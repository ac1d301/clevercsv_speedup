...
static inline bool is_number_with_commas(const std::string& s) {
    ...
    while (i < s.size()) {
        if (std::isdigit((unsigned char)s[i])) {
            ...
            if (i + 1 >= s.size() || !std::isdigit((unsigned char)s[i+1])) return false;
            i++;
        } else {
            break;
        }
...
static bool is_time_str(const std::string& s) {
    ...
    size_t i = 0;
    // HH — validate 00-23
    if (i+1 >= n || !std::isdigit((unsigned char)s[i]) || !std::isdigit((unsigned char)s[i+1])) return false;
    ...
    if (i+1 >= n || !std::isdigit((unsigned char)s[i]) || !std::isdigit((unsigned char)s[i+1])) return false;
    int mm = (s[i]-'0')*10 + (s[i+1]-'0');
    if (mm > 59) return false;
    i += 2;
    // optional :SS — validate 00-59
    if (i < n && s[i] == ':') {
        i++;
        if (i+1 >= n || !std::isdigit((unsigned char)s[i]) || !std::isdigit((unsigned char)s[i+1])) return false;
    ...
    if (i < n) {
        if (s[i] == 'Z') {
            ...
            i++;
            if (i+1 < n && std::isdigit((unsigned char)s[i]) && std::isdigit((unsigned char)s[i+1])) {
                i += 2;
                if (i < n && s[i] == ':') {
                    i++;
                    if (i+1 < n && std::isdigit((unsigned char)s[i]) && std::isdigit((unsigned char)s[i+1]))
...
static bool is_unicode_alphanum_v2(const std::string& s) {
    if (s.empty()) return false;
...
static bool is_known_type(const std::string& cell, bool is_quoted) {
    ...
    if (is_unicode_alphanum_v2(s)) return true;
