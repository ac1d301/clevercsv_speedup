...
v2 patch: compute_type_score now passes raw data + dialect chars directly to
the C++ extension. Parsing happens inside C++ — no Python-side parse_string,
...
class ConsistencyDetector:
    ...
    def compute_type_score(
        ...
        """Compute the type score — patched to use C++ v2 extension.

        v2 passes raw `data` plus the dialect chars to C++. The extension parses
        the CSV internally, eliminating the Python parse_string + tuple-list
        marshaling that dominated v1's runtime.
        """
        import sys
        if '/content' not in sys.path:
            sys.path.insert(0, '/content')
        import type_detector as _td_cpp
        return _td_cpp.bulk_type_score(
            data,
            dialect.delimiter or '',
            dialect.quotechar or '',
            dialect.escapechar or '',
            eps,
        )
